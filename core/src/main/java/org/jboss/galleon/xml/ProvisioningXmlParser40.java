...
public class ProvisioningXmlParser40 implements PlugableXmlParser<ProvisioningConfig.Builder> {
    ...
    enum Attribute implements XmlNameProvider {
        ...
        static {
            ...
            attributes.put(FAMILY_MEMBER_ALLOWED.name, FAMILY_MEMBER_ALLOWED);
    ...
    private static void doReadFeaturePackDep(XMLExtendedStreamReader reader, FeaturePackDepsConfigBuilder<?> builder, boolean transitive)
        ...
        boolean familyMemberAllowed = false;
        final int count = reader.getAttributeCount();
        for (int i = 0; i < count; i++) {
            final Attribute attribute = Attribute.of(reader.getAttributeName(i).getLocalPart());
            switch (attribute) {
                ...
                case FAMILY_MEMBER_ALLOWED:
                    familyMemberAllowed = Boolean.parseBoolean(reader.getAttributeValue(i));
                    break;
                default:
                    throw ParsingUtils.unexpectedContent(reader);
            }
        ...
        depBuilder.setFamilyMemberAllowed(familyMemberAllowed);
        while (reader.hasNext()) {
            switch (reader.nextTag()) {
                case XMLStreamConstants.END_ELEMENT: {
                    try {
